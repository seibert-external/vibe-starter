...
Use dependency injection at system boundaries to make mocking natural.

## This Project's Test Setup

- **Framework**: Vitest (run with `pnpm test` or `pnpm test:watch`)
- **Pattern**: Test tRPC procedures through the caller (public API), not by invoking handler functions directly
